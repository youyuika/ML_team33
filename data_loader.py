...
class Generator(object):
    ...
    def __init__(self):
        ...
        with open("dataset/list/train.txt") as f:
        ...
        # load test set
        self.test_data = []
        with open("dataset/list/test.txt") as f:
    ...
    def Generate(self, sampling_list=None):
        ...
        for start, end in cuts:
            ...
            self.inputs, self.target_lanes, self.target_h, self.test_image, self.data_list = self.Resize_data(
                start, end, sampling_list)
            ...
            yield self.inputs/255.0, self.target_lanes, self.target_h, self.test_image/255.0, self.data_list
    ...
    def Generate_Test(self):
        ...
        for start, end in cuts:
            test_image, path, ratio_w, ratio_h, target_h, target_lanes = self.Resize_data_test(
                start, end)
    ...
    def Resize_data_test(self, start, end):
        ...
        for i in range(start, end):
            ...
            for j in annoatation_data:
                ...
                temp_lanes.append(x)
                temp_h.append(y)
            target_lanes.append(np.array(temp_lanes, dtype=object))
            target_h.append(np.array(temp_h, dtype=object))
    ...
    def Resize_data(self, start, end, sampling_list):
        ...
        test_image = cv2.imread(self.p.test_root_url +
                                self.test_data[test_index][1:-1])
            ...
            if x > 0:
                if p_x < 0:
                    ...
                    for dense_x in range(min(int(p_x), int(x)), max(int(p_x), int(x)), 10):
                        dense_y = p_y - abs(p_x - dense_x) * \
                            abs(p_y-y)/float(abs(p_x - x))
                        if dense_x >= 0 and dense_y >= 0:
                            out_x.append(dense_x)
                            out_y.append(p_y - abs(p_x - dense_x)
                                         * abs(p_y-y)/float(abs(p_x - x)))
                    p_x = x
                    p_y = y
    ...
    def Shadow(self, min_alpha=0.5, max_alpha=0.75):
        ...
        for i in indices:
            ...
            if coin == 0:
                ...
                if rand == 0:
                    vertices = np.array([[top_x, 0], [0, 0], [0, rows], [
                                        bottom_x, rows]], dtype=np.int32)
                elif rand == 1:
                    vertices = np.array([[top_x, 0], [cols, 0], [cols, rows], [
                                        bottom_x, rows]], dtype=np.int32)
                mask = test_image.copy()
                # i.e. 3 or 4 depending on your image
                channel_count = test_image.shape[2]
                ignore_mask_color = (0,) * channel_count
                cv2.fillPoly(mask, [vertices], ignore_mask_color)
                rand_alpha = np.random.uniform(min_alpha, max_alpha)
                cv2.addWeighted(mask, rand_alpha, test_image,
                                1 - rand_alpha, 0., shadow_img)
    ...
    def Translation(self):
        ...
        for i in indices:
            ...
            temp_image = cv2.warpAffine(temp_image, np.float32(
                [[1, 0, tx], [0, 1, ty]]), (self.p.x_size, self.p.y_size))
            temp_image = np.rollaxis(temp_image, axis=2, start=0)
            self.inputs[i] = temp_image
    ...
    def Rotate(self):
        ...
        for i in indices:
            ...
            M = cv2.getRotationMatrix2D(
                (self.p.x_size//2, self.p.y_size//2), angle, 1)

            temp_image = cv2.warpAffine(
                temp_image, M, (self.p.x_size, self.p.y_size))
            temp_image = np.rollaxis(temp_image, axis=2, start=0)
            self.inputs[i] = temp_image
            ...
            for j in range(len(x)):
                ...
                x[j][index_mask], y[j][index_mask] = Rotate_Points(
                    (self.p.x_size//2, self.p.y_size//2), (x[j][index_mask], y[j][index_mask]), (-angle * 2 * np.pi)/360)
                x[j][x[j] < 0] = -2
                x[j][x[j] >= self.p.x_size] = -2
                x[j][y[j] < 0] = -2
                x[j][y[j] >= self.p.y_size] = -2
