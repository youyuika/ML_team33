...
def Training():
    ...
    for epoch in range(p.n_epoch):
        ...
        for inputs, target_lanes, target_h, test_image, data_list in loader.Generate(sampling_list):
            ...
            print("step : " + str(step))
            loss_p = lane_agent.train(
                inputs, target_lanes, target_h, epoch, lane_agent, data_list)
        ...
        if epoch % 1 == 0:
            ...
            for idx in index:
                ...
                test.evaluation(loader, lane_agent, index=idx,
                                name="test_result_"+str(epoch)+"_"+str(idx)+".json")
                # name = "epoch_idx_"+str(epoch) + str(idx) + str(step/1000)
