...
def Testing():
    ...
    if p.mode == 0:  # check model with test data
        ...
        cap = cv2.VideoCapture(
            "/dataset/test_set/driver_37_30frame/05181520_0219.MP4")
        ...
        test_image = cv2.imread(
            p.test_root_url+"driver_37_30frame/05181520_0219.MP4/00000.jpg")
                ...
                if h < min_y:
                    ...
                    for k in range(len(j)-1):
                        if j[k] >= h and h >= j[k+1]:
                            ...
                            if i[k] < i[k+1]:
                                temp_x.append(
                                    int(i[k+1] - float(abs(j[k+1] - h))*abs(i[k+1]-i[k])/abs(j[k+1]+0.0001 - j[k])))
                            else:
                                temp_x.append(
                                    int(i[k+1] + float(abs(j[k+1] - h))*abs(i[k+1]-i[k])/abs(j[k+1]+0.0001 - j[k])))
                            break
                ...
                if h < min_y:
                    ...
                    if last < last_second:
                        l = int(last_second - float(-last_second_y + h) *
                                abs(last_second-last)/abs(last_second_y+0.0001 - last_y))
                        if l > x_size or l < 0:
                            temp_x.append(-2)
                        else:
                            temp_x.append(l)
        ...
        if (np.sum(feature[i]**2)) >= 0:
            ...
            if len(lane_feature) == 0:
                ...
                if min_feature_dis <= p.threshold_instance:
                    lane_feature[min_feature_index] = (lane_feature[min_feature_index]*len(
                        x[min_feature_index]) + feature[i])/(len(x[min_feature_index])+1)
